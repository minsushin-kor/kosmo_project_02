...
describe('getWalkAdviceCopy', () => {
  it('화창한 날에는 반려동물 이름이 포함된 산책 추천 문구를 만든다', () => {
    expect(getWalkAdviceCopy('GOOD', '코코').headline)
      .toBe('코코와 산책하기 딱 좋은 날씨예요!')
  })

  it('휴식이 필요한 날에는 실내 휴식 문구를 만든다', () => {
    expect(getWalkAdviceCopy('REST', '코코').headline)
      .toBe('오늘은 코코와 집에서 포근하게 쉬어가요.')
  })

  it('애매한 날에는 짧고 조심스러운 산책 문구를 만든다', () => {
    expect(getWalkAdviceCopy('CAUTION', '코코').headline)
